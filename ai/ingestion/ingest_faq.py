...
使用方法：
    python -m ai.ingestion.ingest_faq --rebuild   # 三源合并入库
    python -m ai.ingestion.ingest_faq --dry-run   # 预览
    python -m ai.ingestion.ingest_faq --cleanup   # 清理旧集合
"""
import os
import re
import sys
...
async def auto_ingest() -> bool:
    ...
    from ai.config import get_active_faq_collection, _write_active_faq_collection
...
示例:
  python -m ai.ingestion.ingest_faq --rebuild   # 三源合并入库
  python -m ai.ingestion.ingest_faq --dry-run   # 预览合并结果
  python -m ai.ingestion.ingest_faq --cleanup   # 清理旧 FAQ 集合
        """,
    )
...
async def _cleanup_old_faq_collections(keep: int = 1):
    ...
    """
    from ai.config import get_ai_config, get_active_faq_collection
    from qdrant_client import QdrantClient
