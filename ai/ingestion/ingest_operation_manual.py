...
使用方法：
    python -m ai.ingestion.ingest_operation_manual --rebuild   # docx → 入库
    python -m ai.ingestion.ingest_operation_manual --dry-run   # 仅预览切分
    python -m ai.ingestion.ingest_operation_manual -f other.md # 指定源文件
"""
import os
import re
import sys
...
async def auto_ingest() -> bool:
    ...
    from ai.config import _write_active_collection, get_active_collection
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python -m ai.ingestion.ingest_operation_manual --rebuild   # 入库（新集合+自动切换）
  python -m ai.ingestion.ingest_operation_manual --dry-run   # 预览切分
  python -m ai.ingestion.ingest_operation_manual --cleanup   # 清理旧集合
        """,
    )
    parser.add_argument(
        "--file", "-f",
...
async def _cleanup_old_collections(keep: int = 1):
    ...
    """
    from ai.config import get_ai_config, get_active_collection
    from qdrant_client import QdrantClient
