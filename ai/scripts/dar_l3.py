...
JUDGE_PROMPT = (
    ...
    "判断以下几项：\n"
    "intent=用户来意，只看用户开头几条消息的形态，段内后面怎么发展（助手解答、用户要求转"
    "工单、最终生成工单）都不改变它：\n"
    "ticket=反馈腔或委托腔——开口明确要求提单/转人工/报障登记；或开口指向产品本身的毛病"
    "（界面没显示、功能没做、报错、卡顿、需求建议——用户自己改不了，只能提给人做）；"
    "或描述问题后不等解答直接催着处理/提单。这类用户不期待被解答，只想把事情报出去。\n"
    "consult=求助腔——开口指向用户自己的配置或操作（怎么做、什么原因、帮忙看看、有没有"
    "某功能），助手能查资料教他。即使咨询后不满意转了工单、或该话题最终以工单收尾，"
    "来意仍是 consult（属于咨询未解决）。\n"
