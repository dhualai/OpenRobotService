...
_RISK_METRICS: list[MetricDef] = [
    MetricDef(
        ...
        label="风险评估明细",
        desc="逐项目的风险评估结果（风险分数、等级、风险因素），由项目信息（AGV数量/种类、项目类型、人工风险点）与关联工单（未关闭数、近30天新增）按规则综合推算，不读人工登记风险",
        requires_time_range=False,
        output_type=MetricOutputType.LIST,
        collect_fn="_risk_assessment_metric",
    ),
    MetricDef(
        key="risk.by_level",
        dimension=MetricDimension.RISK,
        label="风险等级分布",
        desc="按推算出的风险等级（低/中/高）统计的项目数量分布",
        requires_time_range=False,
        output_type=MetricOutputType.DISTRIBUTION,
        collect_fn="_risk_by_level_metric",
    ),
    MetricDef(
        key="risk.high_risk_count",
        dimension=MetricDimension.RISK,
        label="高风险项目数",
        desc="推算为高风险的项目数量",
        requires_time_range=False,
        output_type=MetricOutputType.SCALAR,
        collect_fn="_risk_high_risk_count_metric",
    ),
]
...
DIMENSION_GROUPS: dict[str, _DimensionGroup] = {
    ...
    "risk": _DimensionGroup(
        label="风险",
        table_name="项目信息+工单推算（不读 risk 表）",
