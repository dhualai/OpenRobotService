...
async def diagnosis_tool_loop_branch(pipe, request, state, memory):
    ...
    system_prompt = (
        ...
        "多份资料都相关时按问题类型选主体：用户问如何完成某个流程"
        "（动作类问题），完整描述该流程步骤的资料是主体；"
        "同设备的单项功能配置/零散问答不进主体，至多结尾一句话提及\n"
        ...
        "必须用 ![说明](url) 引用到**所属步骤**下面——多个步骤各有截图时逐步骤配图，"
        "禁止只配第一张或全部省略；带截图的步骤丢了截图视为不完整回答。"
        "资料里的页面/菜单/按钮名称和参数值照抄原文，禁止换成同义说法。"
        "介绍产品/车型时知识库有图必须引用，不要省略\n"
