...
export default function DataImport() {
  ...
  const [teProjects, setTeProjects] = useState<Project[]>([]);
  const [teProject, setTeProject] = useState<Project | null>(null);
  const [teProjectSearch, setTeProjectSearch] = useState('');
  const [teProjectPickerVisible, setTeProjectPickerVisible] = useState(false);
  ...
  useEffect(() => {
    request('/projects/')
      .then((data) => setTeProjects(normalizeList<Project>(data)))
      .catch(() => { /* 静默失败，切到该 Tab 时不强制要求项目列表已加载 */ });
  }, []);

  const teFilteredProjects = teProjectSearch.trim()
    ? teProjects.filter((p) => {
        const kw = teProjectSearch.trim().toLowerCase();
    ...
    <div style={{ padding: 16 }}>
      <Tabs value={tab} onChange={(v) => setTab(String(v))}>
        ...
        <TabPanel value="transport-efficiency" label="搬运效率导入">
          <div style={{ padding: '24px 0' }}>
            <div
              onClick={() => setTeProjectPickerVisible(true)}
              style={{
            ...
            >
              <div>
                {teProject ? (
                  <div style={{ fontWeight: 500 }}>{teProject.name}</div>
                ) : (
            ...
            <Popup visible={teProjectPickerVisible} onClose={() => setTeProjectPickerVisible(false)} placement="bottom" showOverlay>
              <div style={{ padding: 20, maxHeight: '70vh', overflow: 'auto' }}>
                <h4 style={{ marginBottom: 12 }}>选择项目</h4>
                <Input
                  value={teProjectSearch}
                  onChange={(v) => setTeProjectSearch(String(v))}
                  placeholder="输入项目名称关键词模糊查找"
                  clearable
                  style={{ marginBottom: 12 }}
                />
                {teFilteredProjects.map((p) => (
                  <div
                    key={p.id || p.name}
                    onClick={() => { setTeProject(p); setTeProjectSearch(''); setTeProjectPickerVisible(false); }}
                    style={{
                      background: teProject?.name === p.name ? '#e8f2ff' : '#fff',
                      borderRadius: 8,
                      padding: '12px 14px',
                      marginBottom: 8,
                      cursor: 'pointer',
                      border: teProject?.name === p.name ? '1px solid #0052d9' : '1px solid transparent',
                    }}
                  >
