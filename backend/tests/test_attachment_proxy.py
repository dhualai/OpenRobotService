...
class TestCallAttachmentEndpoint:
    ...
    def test_mov_mime(self, call_client):
        resp = call_client.get(self.URL)
        # iOS Safari/WKWebView 的 <video> 只可靠识别 video/mp4——video/quicktime 会被
        # canPlayType 判为不支持，导致 iPhone 上 .mov 无法内联播放；.mov 内容即 H.264+AAC，
        # 故统一按 video/mp4 返回（且不得落到 application/octet-stream）。
