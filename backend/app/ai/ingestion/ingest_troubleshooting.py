...
读取 backend/docs/问题排查树_v1.json，将每个 symptom 的决策树线性化为
...
使用方法：
    python -m app.ai.ingestion.ingest_troubleshooting --rebuild   # 入库
    python -m app.ai.ingestion.ingest_troubleshooting --dry-run   # 预览
    python -m app.ai.ingestion.ingest_troubleshooting --cleanup   # 清理旧集合
"""
import sys
import json
import hashlib
...
async def _cleanup_old_troubleshooting_collections(keep: int = 1):
    ...
    from app.ai.config import get_ai_config, get_active_troubleshooting_collection
    from qdrant_client import QdrantClient
...
async def auto_ingest() -> bool:
    ...
    from app.ai.config import _write_active_troubleshooting_collection
