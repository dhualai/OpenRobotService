...
导出：
    python -m app.ai.ingestion.snapshot_manager export --dir ./snapshots
    → 对 operation_docs / faq / troubleshooting 三个活跃 collection 各导出一个 .snapshot 文件

导入：
    python -m app.ai.ingestion.snapshot_manager import --dir ./snapshots
    → 读取目录下所有 .snapshot 文件，恢复到 Qdrant 并自动设置活跃指针

用法：
    python -m app.ai.ingestion.snapshot_manager export          # 导出到默认目录
    python -m app.ai.ingestion.snapshot_manager import --dir ./snapshots  # 从指定目录导入
    python -m app.ai.ingestion.snapshot_manager list            # 列出活跃 collection 及快照
    python -m app.ai.ingestion.snapshot_manager --help
"""
import sys
import json
import time
...
DEFAULT_SNAPSHOT_DIR = _backend_dir / "app" / "snapshots"
...
示例:
  python -m app.ai.ingestion.snapshot_manager export --dir ./snapshots
  python -m app.ai.ingestion.snapshot_manager import --dir ./snapshots
  python -m app.ai.ingestion.snapshot_manager list
        """,
    )
