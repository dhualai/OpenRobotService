...
class AiDiagnosisPlatform:
    ...
    async def submit(self, session_id: str) -> dict:
        ...
        db_id = 0
        try:
            # 复用主项目统一任务模型 Task（source='ai'），AI 专属字段入 metadata_info
            from app.models.task import Task, TaskType, TaskPriority, TaskStatus
            from app.core.database import SessionLocal
            # priority 中文 -> 枚举；type 值与 TaskType 完全一致(problem/bug/feature/support/other)
            _pri = {"紧急": TaskPriority.URGENT, "高": TaskPriority.HIGH,
                    "中": TaskPriority.MEDIUM, "低": TaskPriority.LOW}
            _typ = {t.value: t for t in TaskType}
            db = SessionLocal()
            record = Task(
                title=(ticket.get("title", "") or "")[:255],
                description=ticket.get("description", "") or "",
                task_type=_typ.get(ticket.get("type", "other"), TaskType.OTHER),
                status=TaskStatus.NEW,
                priority=_pri.get(ticket.get("priority", "中"), TaskPriority.MEDIUM),
                created_by="ai",                        # NOT NULL；AI 工单无发起人，用常量标记
                source="ai",                            # 复用外部源机制，区分 AI 工单
                external_id=ticket.get("ticket_id") or None,  # 空→NULL，避免 (source,external_id) 唯一约束冲突
                attachments=ticket.get("attachments", []),
                metadata_info={                         # AI 专属字段统一入 JSON
                    "session_id": session_id,
