...
# iOS Safari / WKWebView 的 <video> 只可靠识别 video/mp4——对 video/quicktime 的
# canPlayType 返回空串（判为不支持），导致 iPhone 上 .mov 视频无法内联播放（表现为
# 无法打开/斜线图标）。.mov/.m4v 的实际内容即 H.264+AAC（iPhone 相机/屏幕录制），
# AVPlayer 对 QuickTime 容器原生可解码，故统一按 video/mp4 返回，让 iOS 走视频加载路径。
