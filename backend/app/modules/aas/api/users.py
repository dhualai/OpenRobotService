...
from app.modules.aas.schemas.user import User, UserCreate, UserUpdate, UserDetail
from app.modules.aas.schemas.role import RoleBatchRemoval, RoleAssignment
from app.modules.aas.schemas.project import ProjectUserRoleAssignment
...
async def get_users(
    ...
    current_user: UserModel = require_permission("backend:user:base:read")
):
...
async def create_user(
    ...
    current_user: UserModel = require_permission("backend:user:base:write")
):
    ...
    return User(
        ...
        projectPermissions=getattr(created_user, 'projectPermissions', {}),
        name=getattr(created_user, 'name', None),
        status=getattr(created_user, 'status', 'inactive'),
        external_credentials=getattr(created_user, 'external_credentials', {})
    )
...
async def get_user_detail(
    ...
    return UserDetail(
        ...
        roles=user.roles,
        projectPermissions=getattr(user, 'projectPermissions', {}),
        name=getattr(user, 'name', None),
        status=getattr(user, 'status', 'inactive'),
        external_credentials=getattr(user, 'external_credentials', {})
    )
...
async def update_user(
    ...
    return User(
        ...
        name=getattr(updated_user, 'name', None),
        status=getattr(updated_user, 'status', 'inactive'),
        external_credentials=getattr(updated_user, 'external_credentials', {})
    )

@router.delete("/{username}", response_model=SuccessResponse)
async def delete_user(
    username: str,
    current_user: UserModel = require_permission("backend:user:base:delete")
):
    user = db_manager.get_user(username)
    if not user:
        raise HTTPException(
...
async def assign_role(
    ...
    current_user: UserModel = require_permission("backend:user:role_project:write")
):
    user = db_manager.get_user(username)
    if not user:
