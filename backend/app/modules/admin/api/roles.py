...
async def get_roles(
    current_user: Dict[str, Any] = require_permission("backend:role:base:read")
):
    try:
...
async def add_permissions_to_role(
    ...
    current_user: Dict[str, Any] = require_permission("backend:role:permission:write")
):
    role = db_manager.get_role(role_id)
    if not role:
        raise HTTPException(status_code=404, detail="角色不存在")
...
async def delete_role(
    role_id: str,
    current_user: Dict[str, Any] = require_permission("backend:role:base:delete")
):
    role = db_manager.get_role(role_id)
    if not role:
        raise HTTPException(status_code=404, detail="角色不存在")
