...
> 已确认：派单采用后端“指定处理人”强信号机制，U1 在固定问答中明确提单给 U2
...
5. 固定问答脚本中，U1 会明确表达“请把工单提给 U2 显示姓名”。
...
说明：固定问答脚本的具体问题与回复，取决于可控 AI 回复实现，本规范先把对话轮次定义成“U1 补齐必填字段直到可提单”，具体文本待环境确认后固化。U1 在其中一条固定回复中明确“请把工单提给 U2 显示姓名”，AI 应把该信息写入 `requested_assignee`，草稿描述以 `[指定处理人：U2显示姓名]` 开头。
...
| S8 | 若返回 `need_fields`，U1 回对话补充，再点转工单 | U1 | 重复 S6-S7 | 补充内容 | `stage=draft_ready`，弹窗展示草稿；描述包含 `[指定处理人：U2显示姓名]` |
...
3. S7-S8：`prepare` 返回的草稿描述以 `[指定处理人：U2显示姓名]` 开头。
4. S10：确认提单后工单已入库，`created_by=U1`，`status=new`，`source=ai`，`assigned_to` 为空。
